...
if __name__ == '__main__':
    ...
    if args.eval:
        ...
        if args.inc_path:
            ...
            for name, path in utils.data_loaders_names.items():
                for severity in range(1, 6):
                    ...
                    test_stats = evaluate(inc_data_loader, model, device, epoch)
        ...
        if args.ina_path:
            ...
            test_stats = evaluate(ina_data_loader, model, device, epoch, mask=imagenet_a_mask)
        ...
        if args.inr_path:
            ...
            test_stats = evaluate(inr_data_loader, model, device, epoch, mask=imagenet_r_mask)
        ...
        if args.insk_path:
            ...
            test_stats = evaluate(insk_data_loader, model, device, epoch)
            print(f"Accuracy on the ImageNet-Sketch: {test_stats['acc1']:.1f}%")

        if args.fgsm_test:
            test_stats = evaluate(data_loader_val, model, device, epoch, adv='FGSM')
